...
# --------------------------------------------------------------------------- #
# 20. A non-golden corridor completes with an honest trace (§6/§24).
# Post-B integration: SpatialTransitGraph may generate candidates for arbitrary
# corridors, so recommendation may or may not be None depending on whether the
# graph can connect the origin and destination.
# --------------------------------------------------------------------------- #
def test_20_unknown_corridor_recommends_nothing() -> None:
    ...
    # With Workstream B, the spatial graph may generate candidates for arbitrary corridors.
    # The key invariant is that the agent completes honestly — no fabricated data.
    search_actions = [
        a
        for a in context.actions
    ...
    if context.recommendation is not None:
        # Workstream B generated a route — verify it's honest data, not fabricated
        assert context.recommendation.id  # has a valid identifier
        assert context.recommendation.data_source is not None
    else:
        # No route found — verify honest explanation
